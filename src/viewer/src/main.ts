...
const MODEL_ID_FROM_URL = urlParams.get('model');
const EMBED_MODE = urlParams.get('embed') === 'true';
const DEFAULT_MODEL_ID = 'a0e17b63-ee8b-44e2-bbe6-01461a29c80c';

// Use URL model ID or default
const CURRENT_MODEL_ID = MODEL_ID_FROM_URL || DEFAULT_MODEL_ID;

console.log('🎯 Viewer initialized with model ID:', CURRENT_MODEL_ID);
console.log('📺 Embed mode:', EMBED_MODE);

// Load models from backend API and local files
const modelFiles = [
  // Local test models (fallback)
  { path: "../outputFrag/test.frag", name: "Test Model", type: "local" },
  // Backend uploaded model (dynamic based on URL)
  { modelId: CURRENT_MODEL_ID, name: "Uploaded Model", type: "backend" },
];  
...
const loadModels = async () => {
  ...
  console.log("🎯 Target Model ID:", CURRENT_MODEL_ID);
  ...
  for (const modelFile of modelFiles) {
    try {
      ...
      if (modelFile.type === "backend") {
        // Load from backend API
        console.log(`Loading model ${modelFile.modelId} from backend...`);
        
        // Get model metadata
        const modelResponse = await fetch(`${API_BASE}/api/models/${modelFile.modelId}`);
        if (!modelResponse.ok) {
          throw new Error(`Failed to fetch model metadata: ${modelResponse.statusText}`);
        }
        
        const modelData = await modelResponse.json();
        console.log('Model metadata:', modelData);
        
        if (modelData.model.status !== 'ready') {
          throw new Error(`Model not ready. Status: ${modelData.model.status}`);
        }
        
        if (!modelData.model.downloadUrl) {
          throw new Error('No download URL available for model');
        }
        
        // Download the FRAG file
        console.log(`Downloading FRAG file: ${modelData.model.originalFilename}`);
        const fragResponse = await fetch(modelData.model.downloadUrl);
        if (!fragResponse.ok) {
          throw new Error(`Failed to download FRAG file: ${fragResponse.statusText}`);
        }
        
        buffer = await fragResponse.arrayBuffer();
        modelName = modelData.model.originalFilename;
        console.log(`✅ Backend model loaded: ${modelName} (${buffer.byteLength} bytes)`);
        
      } else {
        // Load local file
        console.log(`Loading local model: ${modelFile.path}`);
      ...
      // Load into TOE fragments
      const model = await fragments.load(buffer, { modelId: modelName });
      models.set(modelName, model);
      console.log(`🎯 Model added to scene: ${modelName}`);
      
    } catch (error) {
      console.warn(`❌ Could not load ${modelFile.name || modelFile.path}:`, error);
    }
  }
