...
interface ProcessingStatus {
  ...
  jobId?: string // Frontend uses 'jobId' for polling
...
export function MultiFileModelCreation({ onProjectCreated, onClose }: MultiFileModelCreationProps) {
  ...
  // Processing status (same as single file upload)
  ...
  // Poll for multi-file processing status (matching single file pattern)
  useEffect(() => {
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (response.ok) {
            ...
            // Update processing status (same as single file upload - direct assignment)
  ...
  }, [processingStatus?.jobId, processingStatus?.id, processingStatus?.status]) // Match single file pattern - depend on both ID formats and status
  ...
  const handleFileChange = (files: FileList) => {
    ...
    // Auto-generate project name from first file if empty
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add files with their categories
      ...
      // Use XMLHttpRequest for upload progress tracking (same as single file upload)
      const result = await new Promise<any>((resolve, reject) => {
        const xhr = new XMLHttpRequest()

        // Track upload progress (with fallback for missing e.total)
        let lastProgressUpdate = 0
        xhr.upload.addEventListener('progress', (e) => {
          // Calculate total size from all selected files as fallback
          ...
          // Throttle updates to avoid overwhelming React (update every 1% or every 100ms)
          const now = Date.now()
          if (percentComplete !== lastProgressUpdate && (percentComplete % 1 === 0 || now - lastProgressUpdate > 100)) {
            ...
            // Force immediate state updates with visual feedback
            ...
            // Debug: Force DOM update by finding progress bar and updating directly
            setTimeout(() => {
        ...
        xhr.addEventListener('load', () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            // Ensure 100% is shown before switching to processing
      ...
      // Check if we have a valid jobId before starting polling
      if (result.jobId) {
        ...
        // Start processing status polling (matching single file pattern)
        setProcessingStatus({
          jobId: result.jobId,
          status: 'processing',
  ...
  // Processing Modal for real-time status updates (copied from single file upload)
  ...
  return createPortal(
    <>
      {/* Only show the main modal if processing modal is not active and not processing in background */}
