...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] relative">
      ...
      <div className="fixed top-[73px] left-0 bottom-0 z-40 w-64 hidden lg:block">
        <Sidebar />
      </div>
      
      {/* Mobile Sidebar Overlay - Shows when menu is open */}
      <div className="fixed inset-0 z-30 lg:hidden" id="mobile-sidebar-overlay" style={{ display: 'none' }}>
        <div className="absolute inset-0 bg-black/50" id="mobile-sidebar-backdrop"></div>
        <div className="absolute top-[73px] left-0 bottom-0 w-64">
          <Sidebar />
        </div>
      </div>
      
      {/* Main Content with responsive margins */}
      <main className="mt-[73px] lg:ml-64 min-h-[calc(100vh-73px)]">
